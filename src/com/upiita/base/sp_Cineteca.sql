...
@TituloO varchar(200), @TituloE varchar(200), @A�o int, @Director varchar(100), @Pais varchar(50) AS
BEGIN
	DECLARE @IdDirector int
	DECLARE @IdPais int
	...
	END
	BEGIN TRAN
		INSERT INTO Pelicula VALUES (@IdPelicula,@TituloO,@TituloE,@A�o,1)
...
@IdPelicula varchar(20),@TituloO varchar(200), @TituloE varchar(200), @A�o int, @Director varchar(100), @Pais varchar(50), @DirectorActual varchar(100), @PaisActual varchar(50)  AS
BEGIN
	...
	BEGIN TRAN
		...
		IF(@A�o <> (SELECT a�o FROM Pelicula WHERE idPelicula=@IdPelicula))
			UPDATE P SET [a�o]=@A�o FROM Pelicula AS P WHERE idPelicula=@IdPelicula 
		IF(@Pais <> @PaisActual)
...
CREATE OR ALTER PROCEDURE usp_BajaPelicula @TituloO varchar(200), @A�o int AS
BEGIN
	UPDATE P SET [estado] = 0 FROM Pelicula as P WHERE tituloOriginal=@TituloO AND a�o=@A�o
END
...
CREATE OR ALTER PROCEDURE usp_ReactivarPelicula @TituloO varchar(200), @A�o int AS
BEGIN
	UPDATE P SET [estado] = 1 FROM Pelicula as P WHERE tituloOriginal=@TituloO AND a�o=@A�o
END
...
SELECT P.idPelicula,P.tituloOriginal,P.tituloExhibicion,P.a�o,P.estado,PA.nombre Pais,D.nombre Director FROM Pais PA 
...
EXEC usp_AltaPelicula 'evangelion','Evangelion',1997,'ABEL GANCE','M�XICO'
...
select * from vReporte
EXEC usp_EditarPelicula 'C-13195','Evangelion 3.0+1.0','Evangelion',2018,'JAFAR PANAHI','M�XICO','NURI BILGE CEYLAN','MACEDONIA'
EXEC usp_EditarPelicula 'C-13199','AUF DER SUCHE NACH INGMAR BERGMAN','BUSCANDO A INGMAR BERGMAN',2017,'JEAN-LUC GODARD','M�XICO','FELIX MOELLER','FRANCIA'
*/